...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#staticBackdrop" onclick="loadDetails('${product.id}')">Details</button></div>
      `;
...
const loadDetails = (id) => {
  ...
  const detailsParent = document.getElementById('modal');
  detailsParent.textContent=''
  data.forEach(x => {
    if (x.id == id) {
      
      const div = document.createElement('div');
     div.innerHTML=`<div class ='postion'  id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
    <div class="modal-content">
    <div class="modal-header" id='toggle'>
    <small id='toggle-button'>X</small>
    <img src="${x.image}" class='product-image-details' alt="">
      <h5 class="modal-title" id="exampleModalLabel">${x.title}</h5>
    </div>
    <div class="modal-body">
      <p>${x.description} </p>
      <small>Catagory: ${x.category}</small>
      <small>Rating: ${x.rating.rate}</small>
    </div>
  </div>
 </div>
  </div>`
